...
The `config.nu` uses Chezmoi templates to automatically generate `source` commands for `.nu` modules and the templated cache loader:
...
This ensures the configuration works across different systems, even when modern tools (eza, bat, fd, ripgrep) aren't installed.
...
### File Operations
- `ls`, `ll`, `la`, `lt` - List files (uses eza if available)
...
All integrations check for command availability before initialization. Generated init scripts are cached in `~/.cache/nushell-init` and sourced by `autoload/07-source-tools.nu`.
