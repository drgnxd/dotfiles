...
`env.nu` and `config.nu` source modules using a pinned `config_dir` constant (currently `/Users/author/.config/nushell`) to avoid const-evaluation errors from `$nu.config-path`/`$nu.env-path`:

```nushell
# env.nu
const config_dir = '/Users/author/.config/nushell'
