...
`env.nu` and `config.nu` source modules via pinned absolute paths (`/Users/author/.config/nushell/...`) so lookups stay stable even when loaded config files live in Home Manager store paths:

```nushell
# env.nu
source '/Users/author/.config/nushell/autoload/01-env.nu'
source '/Users/author/.config/nushell/autoload/02-path.nu'
