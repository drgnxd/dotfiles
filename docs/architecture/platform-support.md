...
- Stats configuration via Home Manager
- Home Manager activation disables legacy app-managed LaunchAgents (Stats/Maccy) to avoid duplicate launches
...
**Desktop stack**:
- Hyprland — tiling window manager (SchemeSpoon equivalent)
...
**Keybinding parity**: Window management keybindings (`Ctrl+Alt` + arrows/`U``I``J``K`/`C`/`Enter`) are mirrored from macOS SchemeSpoon for consistent muscle memory.
...
**Not available on Linux**: macOS system defaults, LaunchAgents, Homebrew casks, and direct SchemeSpoon APIs.
