...
- Stats の設定は Home Manager で管理
- Home Manager の activation で、Stats/Maccy の旧アプリ管理 LaunchAgent を無効化し二重起動を防止
...
**デスクトップスタック**:
- Hyprland — タイル型ウィンドウ管理（SchemeSpoon 相当）
...
**キーバインド互換**: ウィンドウ操作の `Ctrl+Alt` + 矢印/`U``I``J``K`/`C`/`Enter` は macOS の SchemeSpoon 設定と揃え、筋肉記憶を維持します。
...
**Linux で未提供**: macOS system defaults、LaunchAgents、Homebrew casks、SchemeSpoon の直接 API。
