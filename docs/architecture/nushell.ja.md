...
`config.nu`はChezmoiテンプレートを使用して`.nu`モジュールとキャッシュ読み込み用の`source`コマンドを自動生成します：
...
これにより、モダンなツール（eza、bat、fd、ripgrep）がインストールされていない異なるシステムでも設定が機能します。
...
### ファイル操作
- `ls`, `ll`, `la`, `lt` - ファイル一覧（使用可能な場合はezaを使用）
...
全ての統合は、初期化前にコマンドの存在を確認します。生成された初期化スクリプトは`~/.cache/nushell-init`にキャッシュされ、`autoload/07-source-tools.nu`で読み込みます。
