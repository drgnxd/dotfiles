...
**Starship の安全重視プロンプト**:
- プロンプトは両端を丸く揃えた Solarized Dark の静かな `base02` Neutral Rail（共有 carrier 上に blue の location、green の VCS、cyan の environment を前景色で表示し、orange の dirty state と red の anomaly は該当時だけ chip として表示）で、通常時は意図的に静かに保つ。左側には、必要な場合のホスト名/ユーザー名、現在地、Git の状態、Nix shell、Direnv、仮想環境、SSH agent の異常だけを表示
...
- `hosts/darwin/default.nix` がシステム設定を管理
- `hosts/darwin/launchd.nix` が launchd エージェント/サービスを管理
...
- `home/modules/secrets.nix` の home-manager layer で定義し、macOS と Linux の両方で利用可能にして、activation 時にユーザー設定パスへ復号配置
- `secrets/secrets.nix` は CI 互換のため空鍵の追跡デフォルトを維持し、実運用の受信者設定テンプレートは `secrets/secrets.example.nix` を使用
