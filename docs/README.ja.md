...
テンプレートをコピーしてマシン固有の識別情報を設定してください。`preferences.nix` は任意であり、作成しない場合は可搬性を優先した既定値を使います。
...
項目とプラットフォーム別の確認コマンドは [`local/README.md`](../local/README.md) を参照してください。
...
- `package.json` は、ローカル管理 plugin に必要な plugin SDK 依存 (`@opencode-ai/plugin`) だけを固定します。これ自体は OpenCode plugin を有効化しません。
- ローカル plugin を追加または更新する場合は、パッケージ依存と lockfile を固定し、`opencode.local.json` に設定を加えてから home-manager activation package を再ビルドしてください。
...
ローカルの pre-commit フックは flake から生成します。clone ごとに一度 dev shell へ入るとインストールされます：

```sh
nix develop
```

個別の pre-commit YAML 設定ファイルは編集しません。CI は同じ formatter と hook derivation を `checks.x86_64-linux.{formatting,pre-commit-check}` で実行します。
